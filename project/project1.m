...
while (transmitted_u<=120||transmitted_d<=400)
    direction=2*pi*rand;
    v=1+4*rand;
    t=1000*randi(6);
    H(count,1)=m_x;
    ...
    for j=1:19
        if d_present>sqrt((BS_x(j)-(m_x))^2+(BS_y(j)-(m_y))^2)
            ...
            if transmitted_u<120
            transmitted_u=0;
            end
            if transmitted_d<400
            transmitted_d=0;
            end
        end
    end
    if (mod(i,5)>=3||mod(i,5)==0)% change based on which configuration
    ...
    if transmitted_u>120 && transmitted_d>400
    break;
    end
