...
sum=0;
for j=1:18
    d_center=sqrt(BS_x(j)^2+BS_y(j)^2);
    P_tmp=10*log10((h_t*h_t)^2/(d_center)^4)+Pt_b+G_t+G_r;
    sum=sum+10^(P_tmp/10);
end
P_du=sum;%W
...
while (transmitted_u<=40||transmitted_d<=800)
    ...
    for i=1:19
    for j=1:50
        direction_m(i*50+j-50)=2*pi*rand;
        v_m(i*50+j-50)=rand*4+1;
    end
    end
    for i=1:t
    ...
    for k=1:19
    for j=1:50
        dir_tmp=direction_m(k*50+j-50);
        x(k*50+j-50)=x(k*50+j-50)+v_m(k*50+j-50)*sin (dir_tmp)/1000;
        y(k*50+j-50)=y(k*50+j-50)+v_m(k*50+j-50)*cos (dir_tmp)/1000;
    end
    end
    ...
    d(50*j+k-50)=sqrt((x(50*j+k-50)-BS_x(j))^2+(y(50*j+k-50)-BS_y(j))^2);
    end
    end

    for j=1:19
    for k=1:50
    P_r(50*j+k-50)=10*log10((h_t*h_r)^2/(d(50*j+k-50))^4)+Pt_m+G_t+G_r;
    end
    end
    ...
    if (mod(i,5)==3)% change based on which configuration
        
    
        Pr_b=10*log10((h_t*h_r)^2/(d_present)^4)+Pt_m+G_t+G_r;
        cantransmit=0;
        
            if Pr_b<10*log10(P_du+Noise_b(cellid))%if noise>signal, can't send
                cantransmit=1;
                busy=busy+1;
            end
    ...
    elseif(mod(i,5)<=1||mod(i,5)==4)
        ...
        for j=1:19
            ...
            for k=1:50    
                d_m=sqrt((x(50*j-50+k)-m_x)^2+(y(50*j-50+k)-m_y)^2);
                Pud=10*log10((h_r*h_r)^2/(d_b)^4)+Pt_m+G_t+G_r;%noise of other mobile
                sum=sum+10^(Pud/10);
            end
            end
        end
